...
**15:00–17:00, room A330 (Idem-Lab).** This page is the whole plan. Two parts: first you give yourself a **tutor** (30 minutes, together), then you work through **three exercises** at your own pace, with the lesson pages open and the tutor beside you.
...
[pi](pi.md) is a coding agent that runs in your terminal and talks to the university's own language models: free, and nothing leaves Strasbourg. Today you use it as a **tutor**: it reads your files, explains what is on your screen and gives hints, and it cannot change anything.
...
Follow [pi, section 2](pi.md#2-install-pi): pick the tab of your machine (WSL / Linux, Mac, Git Bash, Codespaces). Then, in a **new** terminal:
...
Follow [pi, section 3](pi.md#3-connect-pi-to-the-universitys-models). It is one download, then:
...
Get it, once ([pi, section 5.1](pi.md#51-install-it)):

```bash title="Any terminal"
pi install git:github.com/KnuxV/cours-agents
```

Run it, every time, **from the folder of the exercise you are working on** ([pi, section 5.2](pi.md#52-use-it)):
...
The exercises run Python programs with `uv`, which also installs Python for you. Follow [Session 2, section 3](sessions/s2.md#3-install-uv) (one command), then, in a **new** terminal:
...
|---|---|
| `pi: command not found`, `No models available`, `401`, `/skill:tutor` not offered | [pi — When things go wrong](pi.md#6-when-things-go-wrong) |
| `uv: command not found`, `No module named …` | [Session 2 — When things go wrong](sessions/s2.md#9-when-things-go-wrong) |
