...
GitHub does not accept your account password from `git`. The path we use is the **GitHub CLI**, `gh`, which logs you in through the browser once and then lets `git` reuse that login. (Fallback if it fails: a *personal access token* generated on GitHub, pasted as the password when `git push` asks for one — come and see us.)
