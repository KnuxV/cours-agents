...
**Two hours in class.** The first session of four; [Session 0 — Setup](../setup.md) is the homework that comes before it.
...
!!! example "The exercises of this session"
    One page each: [✏️ 1.1 Recipe history](../exercises/recipe.md) · [✏️ 1.2 Scrabble](../exercises/scrabble.md) · [✏️ 1.3 Collaboration, in teams of two](../exercises/collab.md) — and, if you are ahead, [three merges in a row](../exercises/scrabble-three-merges.md).

    The class plan for the practice half, with the tutor: [tutor, then three exercises](practice-git.md). Reference material this session leads to: [Git as a collaboration tool](../reference/git-collaboration.md).
...
Collaboration — forks, pull requests, issues, CI — is **not** covered in class: it is the reading [Git as a collaboration tool](../reference/git-collaboration.md), to do at home before Session 2.
...
=== "Linux"
    ...
    Installed on the university desktops. On your own laptop, if it says `command not found`, install it with your package manager (Ubuntu/Debian: `sudo apt install git`) — see [Path E](../setup.md#path-e-linux).
...
```bash title="Any terminal (WSL / Mac / Linux / Git Bash / Codespace)"
...
!!! tip "If you unzipped by hand on Windows"
    The folder is probably in `Downloads`. From WSL, that is `cd /mnt/c/Users/<YourWindowsName>/Downloads/practice-site`; from Git Bash, `cd ~/Downloads/practice-site`. Better: move the folder to your home first (see [where to keep your work](../reference/wsl.md#where-to-keep-your-work)).
...
=== "Linux"
    ...
    (Or open the file from the browser's menu: ++ctrl+o++, then navigate to `practice-site`.)
...
Open `index.html` in an editor (`nano index.html` or `micro index.html` in the terminal, or VS Code — see [Editing files](../reference/wsl.md#editing-files) if you are on WSL). Find the line
...
You will meet one in [exercise 1.2, Scrabble](../exercises/scrabble.md), and another, with a classmate, in [exercise 1.3](../exercises/collab.md). Conflicts are not failures: they are Git refusing to guess.
...
cd agent-lab
git init
echo "# agent-lab — Meta-programming course" > README.md
...
=== "Linux"
    ...
    On the university desktops there is no `sudo`, so no `apt install`. Download `gh` into your home folder instead — no administrator rights needed (the first line asks GitHub which version is current, the second downloads and unpacks it into `~/.local`, the third makes `~/.local/bin` part of the places your shell looks for programs):

    ```bash title="Terminal (Linux)"
    GH_VER=$(curl -s https://api.github.com/repos/cli/cli/releases/latest | grep tag_name | cut -d '"' -f 4 | tr -d v)
    mkdir -p ~/.local && curl -sL "https://github.com/cli/cli/releases/download/v${GH_VER}/gh_${GH_VER}_linux_amd64.tar.gz" | tar xz -C ~/.local --strip-components=1
    echo 'export PATH="$HOME/.local/bin:$PATH"' >> ~/.bashrc && source ~/.bashrc
    gh --version
    gh auth login
    ```

    On your own laptop with `sudo`, the WSL tab's `apt` line works on Ubuntu/Debian; other distributions: [cli.github.com](https://cli.github.com/) lists the package for each.
...
`origin` is the conventional name for "the copy on GitHub" — the reading [Git as a collaboration tool](../reference/git-collaboration.md#1-remotes-origin-is-just-a-name) explains remotes properly. `-u` ties your local `main` to it, so that from now on plain `git push` and `git pull` know where to go. Refresh the GitHub page: your README is there.
...
One page per exercise, with expected outputs and a folded solution.

1. [**Recipe history**](../exercises/recipe.md) — clone `KnuxV/recipe-history`, read eight commits like a detective, restore one quantity without losing what came after. First contact with `git clone` and `git remote -v`.
2. [**Scrabble: two features, one conflict**](../exercises/scrabble.md) — merge two branches written by two developers; the second merge stops on a conflict that you resolve by hand.
3. [**Collaboration: two people, one repository**](../exercises/collab.md) — in teams of two: a fork, a pull request, and the conflict that follows.

At home: the reading [Git as a collaboration tool](../reference/git-collaboration.md).
