...
- One branch per feature, merged before the next one starts (exercise 2.3 part C).
...
- **1.3 Branches: fast-forward, merge commit, conflict**, inside `agent-lab`, starting on `main` with a clean `git status`. Part A: branch `add-notes`, commit, merge; `git merge` prints `Fast-forward` and the graph is a straight line. Part B: branch `longer-readme`, plus a commit on `main` touching another file; the merge makes a merge commit and the graph shows a fork that rejoins. Part C (stretch): both sides edit the same line; the student resolves the conflict by hand. Done when the student can explain, from their own graph, why A made no new commit and B did.
